...
\begin{figure}[ht!]
	...
	\begin{tikzpicture}
		...
		% Pivoting
		\draw [decorate, decoration = {brace, raise=0.7cm, amplitude=6pt}, thick] (nj.south) to node [midway, left=0.9cm] (argmax) {$p = \argmax\limits_{j \leq k \leq n} \left| l_{k, j} \right|$} (jj.north);
		\draw[arrow_head-arrow_head, decorate, decoration={}, bend left, out=140, in=140] (argmax.west) to node [midway, rotate=28, above=-1cm, right=-1cm] {Swap rows $j$ and $p$} (jj.west);
		
		% Labels		
		\node[text_label, above=5mm of {$(jj2.north west)!0.5!(jdot.north east)$}] 			 {Prior Iterations};
		\node[text_label, left=4.5cm of {$(dotj.south west)!1.4!(j2j.north west)$}, rotate=90] {Prior Iterations};
...
Unlike direct methods, iterative methods improve an initial estimate of a solution until a desired accuracy is reached.
...
As detailed in \citetitle{Cejka2022} \cite{Cejka2022}, the complete-factorization approach iteratively refines the initial estimates of $\mathbf{L}$ and $\mathbf{U}$ until the desired accuracy of $\mathbf{A} = \mathbf{LU}$ is reached.
...
While the instructions in Steps~\ref{Item:theory->ICMPP->LUP->ICMPP->algorithm-steps->compute-Lt} to \ref{Item:theory->ICMPP->LUP->ICMPP->algorithm-steps->evaluate-processing-rule} must be performed consecutively, the operations in each step can be executed in parallel:
...
Thus, all elements $l^{t}_{i,j}$ can be computed simultaneously.
	\item Step \ref{Item:theory->ICMPP->LUP->ICMPP->algorithm-steps->process-lower-sections} - The absolute-value conditions can be checked in parallel, and, similarly to Step~\ref{Item:theory->ICMPP->LUP->ICMPP->algorithm-steps->compute-Lt}, the iterative processing is computed according to the formula used in the same step.
...
Note that due to the nature of iterative methods, it is not possible to accurately predict the number of iterations needed to iteratively refine the initial estimate until the desired accuracy is reached.
