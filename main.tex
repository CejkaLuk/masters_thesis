...
\documentclass[a4paper, twoside, 12pt, czech, american]{book}
...
\usepackage[backend=biber,								  % In TeXstudio, the backend here must correspond to: Options -> Configure TeXstudio... -> Build -> Default Bibliography Tool
style=iso-numeric,
sorting=none,
giveninits=true]{biblatex} 					 			  % Bibliography: ISO 690 format, cite shows numbers, sort according to occurence in text, shorten first names
...
\DeclareRobustCommand{\code}[1]{{\begingroup\sethlcolor{light-gray}\hl{\texttt{#1}}\endgroup}} % Shortcut for code font in text
\soulregister{\code}{1}
...
% TODO: Replace zadani_cele.pdf with scan of original assignment paper (colored; with all signatures)
% Task paper scanned as a 1-page PDF:
\includepdf[pages={1}]{resources/zadani_cele.pdf}
...
\nocite{*} % Write out all citation in the bibliography - even if they're not cited in the text
